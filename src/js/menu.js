      ...
      if ($().accessibleMegaMenu) {
        ...
        $(context).find(".main-menu__item--submenu-level-1").each(function () {
          $(this).hover(
            function () {
              $(this)
                .children('.main-menu__item-list--level-2')
                .addClass('content-panel-is-open')
                .removeClass('content-panel-is-closed');
              $(this)
                .addClass('link-expanded')
                .removeClass('link-collapsed');
            },
            function () {
              $(this)
                .children('.main-menu__item-list--level-2')
                .removeClass('content-panel-is-open')
                .addClass('content-panel-is-closed');
              $(this)
                .removeClass('link-expanded')
                .addClass('link-collapsed');
            }
          );
        });
        ...
        $(context).find(".main-menu__item--submenu-level-0").once('menu-classes').each(function () {
          ...
          $(this).find('> a').addClass('dropmenu');

          // if Mega menu, add a class to the parent and remove dropmenu which is the default.
...
*/
  Drupal.behaviors.ucsfdOffCanvas = {
    attach: function (context, settings) {
      ...
      if ($().mmenu) {
        const menu = new Mmenu("#block-off-canvas-nav", {
          // Options below.
        ...
        const api = menu.API;
        // Choose the menu trigger.
        const trigger = document.getElementById('off-canvas-opener');
        if (trigger) {
          // onClick function.
        ...
        const menu_item = once('each-menu-item', context.querySelectorAll('.mm-btn > span'));
        for (let i = 0; i < menu_item.length; i++, (event)) {
